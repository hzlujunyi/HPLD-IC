...
echo Compiling HPLDIC in %BUILD_CONFIG% configuration
pushd %BUILD_FOLDER%
...
if !ERRORLEVEL! NEQ 0 goto Error
7z a %SYMBOLS_FOLDER%\HPLDICDebuggingSymbols-%ARCH%-%VERSION%.zip %SYMBOLS_FOLDER%\*.pdb
if !ERRORLEVEL! NEQ 0 goto Error

if "%ML_SYMBOL_STORE%" NEQ "" (
    echo Publishing PDBs to symbol store: %ML_SYMBOL_STORE%
    symstore add /f %SYMBOLS_FOLDER%\*.pdb /s %ML_SYMBOL_STORE% /t HPLDIC
    if !ERRORLEVEL! NEQ 0 goto Error
) else (
...
if "%ML_SYMBOL_ARCHIVE%" NEQ "" (
    ...
    copy %SYMBOLS_FOLDER%\HPLDICDebuggingSymbols-%ARCH%-%VERSION%.zip %ML_SYMBOL_ARCHIVE%
    if !ERRORLEVEL! NEQ 0 goto Error
) else (
    if "%MUST_DEPLOY_SYMBOLS%"=="1" (
...
windeployqt.exe --dir %DEPLOY_FOLDER% --%BUILD_CONFIG% --qmldir %SOURCE_ROOT%\app\gui --no-opengl-sw --no-compiler-runtime --no-qmltooling --no-sql %BUILD_FOLDER%\app\%BUILD_CONFIG%\HPLDIC.exe
if !ERRORLEVEL! NEQ 0 goto Error
...
forfiles /p %DEPLOY_FOLDER% /m *.qml /s /c "cmd /c qmlcachegen.exe --target-architecture=%ARCH% @path"
if !ERRORLEVEL! NEQ 0 goto Error

@REM echo Deleting original QML files
@REM forfiles /p %DEPLOY_FOLDER% /m *.qml /s /c "cmd /c del @path"
@REM if !ERRORLEVEL! NEQ 0 goto Error

echo Harvesting files for WiX
"%WIX%\bin\heat" dir %DEPLOY_FOLDER% -srd -sfrag -ag -sw5150 -cg HPLDICDependencies -var var.SourceDir -dr INSTALLFOLDER -out %BUILD_FOLDER%\Dependencies.wxs
if !ERRORLEVEL! NEQ 0 goto Error

echo Copying application binary to deployment directory
copy %BUILD_FOLDER%\app\%BUILD_CONFIG%\HPLDIC.exe %DEPLOY_FOLDER%
if !ERRORLEVEL! NEQ 0 goto Error
...
if "%ML_SYMBOL_STORE%" NEQ "" (
    ...
    symstore add /r /f %DEPLOY_FOLDER%\*.* /s %ML_SYMBOL_STORE% /t HPLDIC
    if !ERRORLEVEL! NEQ 0 goto Error
)

echo Building MSI
msbuild %SOURCE_ROOT%\wix\HPLDIC\HPLDIC.wixproj /p:Configuration=%BUILD_CONFIG% /p:Platform=%ARCH%
if !ERRORLEVEL! NEQ 0 goto Error

if "%SIGN%"=="1" (
    echo Signing MSI
    signtool %SIGNTOOL_PARAMS% %BUILD_FOLDER%\HPLDIC.msi
    if !ERRORLEVEL! NEQ 0 goto Error
)
...
if !ERRORLEVEL! NEQ 0 goto Error
rem This file tells HPLDIC that it's a portable installation
echo. > %DEPLOY_FOLDER%\portable.dat
if !ERRORLEVEL! NEQ 0 goto Error
7z a %INSTALLER_FOLDER%\HPLDICPortable-%ARCH%-%VERSION%.zip %DEPLOY_FOLDER%\*
if !ERRORLEVEL! NEQ 0 goto Error

echo Build successful for HPLDIC v%VERSION% %ARCH% binaries!
exit /b 0
